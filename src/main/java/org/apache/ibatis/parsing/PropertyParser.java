...
public class PropertyParser {
   ...
   * The special property key that specify a separator for key and default value on placeholder.The default separator is {@code ":"}.
   * @since 3.4.2
   */
  ...
  // 占位符替换处理器（内部私有静态类） VariableTokenHandler实现了TokenHandler接口，包含了一个Properties类型的属性，在初始化这个类时需指定该属性的值
  private static class VariableTokenHandler implements TokenHandler {
    ...
    public String handleToken(String content) {
      ...
      if (variables != null) {
        ...
        // 未开启默认值
        if (variables.containsKey(key)) return variables.getProperty(key);
      }
      // 以上都未匹配返回
      return "${" + content + "}";
    }
