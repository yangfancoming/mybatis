...
public class DefaultVFS extends VFS {
  ...
  public List<String> list(URL url, String path) throws IOException {
    ...
    try {
      ...
      if (jarUrl != null) {
        ...
        try {
          if (isJar(url)) {
            // Some versions of JBoss VFS might give a JAR stream even if the resource referenced by the URL isn't actually a JAR
            is = url.openStream();
            try (JarInputStream jarInput = new JarInputStream(is)) {
              if (log.isDebugEnabled())  log.debug("Listing " + url);
              for (JarEntry entry; (entry = jarInput.getNextJarEntry()) != null; ) {
                if (log.isDebugEnabled()) log.debug("Jar entry: " + entry.getName());
                children.add(entry.getName());
              }
            }
          } else {
            /*
             * Some servlet containers allow reading from directory resources like a text file,listing the child resources one per line.
             * However, there is no way to differentiate between directory and file resources just by reading them.
             * To work around that, as each line is read, try to look it up via the class loader as a child of the current resource.
            ...
            for (String line; (line = reader.readLine()) != null;) {
              if (log.isDebugEnabled()) log.debug("Reader entry: " + line);
              lines.add(line);
    ...
    for (JarEntry entry; (entry = jar.getNextJarEntry()) != null;) {
      if (!entry.isDirectory()) {
        ...
        if (name.indexOf(path) == 0) {
          if (log.isDebugEnabled()) log.debug("Found resource: " + name);
          // Trim leading slash
  ...
  protected URL findJarForResource(URL url) throws MalformedURLException {
    if (log.isDebugEnabled()) log.debug("Find JAR URL: " + url);
    // If the file part of the URL is itself a URL, then that URL probably points to the JAR
    try {
      for (;;) {
        url = new URL(url.getFile());
        if (log.isDebugEnabled()) log.debug("Inner URL: " + url);
      }
    ...
    if (index >= 0) {
      ...
      if (log.isDebugEnabled()) log.debug("Extracted JAR URL: " + jarUrl);
    }else {
      if (log.isDebugEnabled()) log.debug("Not a JAR: " + jarUrl);
      return null;
    }
    ...
    try {
      ...
      if (isJar(testUrl))  return testUrl;
        ...
        if (file.exists()) {
          if (log.isDebugEnabled()) log.debug("Trying real file: " + file.getAbsolutePath());
   ...
   * Returns true if the resource located at the given URL is a JAR file.
   * @param url The URL of the resource to test.
   * @param buffer A buffer into which the first few bytes of the resource are read. The buffer must be at least the size of {@link #JAR_MAGIC}.
  ...
  protected boolean isJar(URL url, byte[] buffer) {
    ...
    try {
      ...
      if (Arrays.equals(buffer, JAR_MAGIC)) {
        if (log.isDebugEnabled())  log.debug("Found JAR: " + url);
        return true;
      }
    } catch (Exception e) {
